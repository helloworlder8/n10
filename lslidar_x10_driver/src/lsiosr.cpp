...
/* 从串口中读取数据 */
// 传入值是字符串指针 长度为1 超时为30                在h文件中默认为30
...
// FD_ZERO() 和 FD_SET() 是 POSIX 标准提供的一组宏，用于操作文件描述符集合 fd_set。
// 将 fdset 清零
    FD_ZERO(&fdset);
  // 将串口的文件描述符加入到 fdset 集合中
    FD_SET(serial, &fdset);
    // 检测文件描述符的状态 serial文件藐视符号  文件描述符集合 可写性 异常性 时间指针
    // 轮询检测文件描述符的状态，以便在有数据可读时进行相应的处理
...
int LSIOSR::init()
{
	int error_code = 0;
	// 打开串口设备文件并获得一个文件描述符号
  // port_.c_str()串口设备文件路径通关ros读取launch文件得到是/ttyusb0
  // 读写 不成为控制终端 非堵塞
